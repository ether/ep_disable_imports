# Contributing to ep_disable_imports

Thanks for helping improve `ep_disable_imports` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_disable_imports` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_disable_imports from your local clone
pnpm run plugins i --path ../ep_disable_imports

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_disable_imports/` are picked up after a server restart.
...
* Etherpad version, ep_disable_imports version, browser + OS
* Steps to reproduce
